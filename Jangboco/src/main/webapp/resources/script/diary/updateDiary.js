...
function loadDiaryImgItemTag() {
	...
	$.ajax({
		...
		success: function(result) {
			...
			for(var i=0; i<result.diaryImgList.length; i++) {
				var diaryImgData = result.diaryImgList[i];
				
				diaryImgArray.push(diaryImgData.IMG_URL);
				
				itemTagArray.push(new Array());
				for(var itemTagData of diaryImgData.itemTagList) {
					var itemTag = new Object();
					itemTag.marketNo = itemTagData.MARKET_NO;
					itemTag.itemsNo = itemTagData.ITEMS_NO;
					itemTag.cost = itemTagData.COST;
					itemTag.buyQnt = itemTagData.BUY_QNT;
					itemTag.marketName = itemTagData.MARKET_NAME;
					itemTag.itemsName = itemTagData.ITEMS_NAME;
					itemTag.addToAccbkFlag = itemTagData.ADD_TO_ACCBK_FLAG;
					
					itemTagArray[i].push(itemTag);
				}
			}
		},
		error: function(request, status, error) {
...
function drawDiaryImgItemTag(diaryImgList) {
	...
	for(var diaryImgData of diaryImgList) {
		thunlHtml += "<li>";
		thunlHtml += "	<div class=\"diary_thunl\" style=\"background:center / cover no-repeat url('resources/upload/";
		thunlHtml += 	diaryImgData.IMG_URL.replace(/\[/g, "%5B").replace(/\]/g, "%5D") + "');\">";
		thunlHtml += "	</div>";
		thunlHtml += "	<button type=\"button\" class=\"del_diary_thunl_btn\" style=\"display:none;\">";
		thunlHtml += "	<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"80%\" height=\"80%\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" class=\"feather feather-trash-2\">";
		thunlHtml += "	<polyline points=\"3 6 5 6 21 6\"></polyline>";
		thunlHtml += "	<path d=\"M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2\"></path>";
		thunlHtml += "	<line x1=\"10\" y1=\"11\" x2=\"10\" y2=\"17\"></line><line x1=\"14\" y1=\"11\" x2=\"14\" y2=\"17\"></line>";
		thunlHtml += "	</svg>";
		thunlHtml += "	</button>";
		thunlHtml += "</li>";
		...
		for(var itemTagData of diaryImgData.itemTagList) {
			...
			itemTagHtml += "	<line x1=\"6\" y1=\"6\" x2=\"18\" y2=\"18\"></line>";
			itemTagHtml += "	</svg>";
			itemTagHtml += "	</button>";
			itemTagHtml += "</li>";
		}
	...
	$("#diary_thunl_list").html(thunlHtml);
...
function drawHastgList(hastgList) {
	...
	for(var hastgData of hastgList) {
		$("#virtual_hastg").text(hastgData.HASTG_NAME);
		var width = $("#virtual_hastg").width() + 46;
	
		html += "<li>";
		html += "	<input type=\"text\" class=\"hastg\" placeholder=\"해시태그\" disabled value=\"" + hastgData.HASTG_NAME + "\" style=\"width:" + width + "px\">";
		html += "	<button type=\"button\" class=\"del_hastg_btn\">";
		html += "		<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"100%\" height=\"100%\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" class=\"feather feather-x\">";
		html += "		<line x1=\"18\" y1=\"6\" x2=\"6\" y2=\"18\"></line>";
		html += "		<line x1=\"6\" y1=\"6\" x2=\"18\" y2=\"18\"></line>";
		html += "		</svg>";
	...
	html += "<li>";
	html += "<input type=\"text\" class=\"hastg\" placeholder=\"해시태그\">";
	html += "<button type=\"button\" class=\"del_hastg_btn\" style=\"display: none;\">";
	html += "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"100%\" height=\"100%\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" class=\"feather feather-x\">";
	html += "<line x1=\"18\" y1=\"6\" x2=\"6\" y2=\"18\"></line>";
	html += "<line x1=\"6\" y1=\"6\" x2=\"18\" y2=\"18\"></line>";
	html += "</svg>";
...
function loadHastgList() {
	...
	$.ajax({
		...
		success: function(result) {
			...
			for(var hastgData of result.hastgList) {
				hastgList.push(hastgData.HASTG_NAME);
			}
		},
		error: function(request, status, error) {
			console.log(error);
